...
module Emarsys
  class Response
    ...
    def initialize(response)
      ...
      if code != 0
        if status == 401
          raise Emarsys::Unauthorized.new(code, text, status, data)
        elsif status == 429
          raise Emarsys::TooManyRequests.new(code, text, status, data)
        else
          raise Emarsys::BadRequest.new(code, text, status, data)
        end
      end
    end
  end
